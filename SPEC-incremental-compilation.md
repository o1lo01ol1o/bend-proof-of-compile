...
- **Reports and the output rule are the checker's.** `poc check` prints `cli_report`'s text on stdout and `poc build` notes it on stderr, from the entry's own definitions (`n0`) plus every non-Base law filled in its import graph, with promises flooded back through the summaries; `poc build` refuses an output that is a loaded file, a foreign file or a directory, with the checker's text.
...
- **Modules: `poc check --module FILE`.** A file checked as an entry is loaded under namespace `""`; imported by a sibling (`import ./X.bend as X`) it is loaded under `X`, so its definitions are `foo` in one role and `X.foo` in the other, and a namespace can change a verdict (a clash with a Base name, a reserved name). An entry's final state is therefore not its importers' prefix, and step keys include the namespace. `--module` loads FILE through the checker's loader under the namespace a sibling importer gives it (the loader's own computation), so its final state is exactly the prefix `P_k` that importer resumes from: checking a chain bottom-up file by file then costs about one cold check of the top entry instead of checking each file twice. Its verdict and diagnostics are the checker's on FILE imported (a one-line sibling root); its report lists FILE's claims under their module names. A module check keeps its own GC root. Reusing an entry's state for its importers instead would need a kernel-certified renaming of the namespace (not taken).
